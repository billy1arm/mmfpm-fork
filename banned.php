...
if ($ban_type === "account_banned")
{
	...
	if(!$sqlr->result($result, 0))
		$sqlr->query("INSERT INTO account_banned (id, bandate, unbandate, bannedby, banreason, active) VALUES ('$entry',".time().",$bantime,'$user_name','$banreason', 1)");
}
else
{
	$sqlr->query("INSERT INTO ip_banned (ip, bandate, unbandate, bannedby, banreason) VALUES ('$entry',".time().",$bantime,'$user_name','$banreason')");
}
if ($sqlr->affected_rows())
	redirect("banned.php?error=3&ban_type=$ban_type");
else
