...
function browse_guilds(&$sqlr, &$sqlc)
{
  ...
  if ($query_myGuild)
  {
    ...
    while ($data = $sqlr->fetch_row($query_myGuild))
    {
      ...
      $output .= ($user_lvl < $owner_gmlvl ) ? "<td>".htmlentities($data[3], ENT_QUOTES, 'UTF-8')."</td>" : "<td><a href=\"char.php?id=$data[2]&amp;realm=$realmid\">".htmlentities($data[3], ENT_QUOTES, 'UTF-8')."</a></td>";
      $output .= "
                <td><img src=\"img/".($data[4]==0 ? "alliance" : "horde")."_small.gif\" alt=\"\" /></td>
                <td>$data[5]/$data[6]</td>
                <td>".htmlentities($data[7], ENT_QUOTES, 'UTF-8')." ...</td>
  ...
  while ($data = $sqlr->fetch_row($query))
  {
    ...
    $output .= "
                <tr>
                  <td>$data[0]</td>";
    $output .= ($user_lvl >= $action_permission['update']) ? "<td><a href=\"guild.php?action=view_guild&amp;realm=$realmid&amp;error=3&amp;id=$data[0]\">".htmlentities($data[1], ENT_QUOTES, 'UTF-8')."</a></td>" : "<td>".htmlentities($data[1], ENT_QUOTES, 'UTF-8')."</td>";
    $output .= ($user_lvl < $owner_gmlvl ) ? "<td>".htmlentities($data[3], ENT_QUOTES, 'UTF-8')."</td>" : "<td><a href=\"char.php?id=$data[2]&amp;realm=$realmid\">".htmlentities($data[3], ENT_QUOTES, 'UTF-8')."</a></td>";
    $output .= "
                  <td><img src=\"img/".($data[4]==0 ? "alliance" : "horde")."_small.gif\" alt=\"\" /></td>
                  <td>$data[5]</td>
...
";
  ...
  while ($member = $sqlr->fetch_row($members))
  {
    ...
    $output .= ($user_lvl < $owner_gmlvl ) ? "
                      <td>".htmlentities($member[1], ENT_QUOTES, 'UTF-8')."</td>" : "
                      <td><a href=\"char.php?id=$member[0]&amp;realm=$realmid\">".htmlentities($member[1], ENT_QUOTES, 'UTF-8')."</a></td>";
    $output .= "
                      ...
                      <td>".htmlentities($member[6], ENT_QUOTES, 'UTF-8')." (".$member[5].")</td>
                      <td>".htmlentities($member[7], ENT_QUOTES, 'UTF-8')."</td>
                      <td>".htmlentities($member[8], ENT_QUOTES, 'UTF-8')."</td>
